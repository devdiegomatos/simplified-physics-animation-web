...
O algoritmo de Expansão de Politopos (EPA, do inglês \textit{Expanding Polytope Algorithm}) surge no contexto da evolução dos métodos de detecção de colisão entre objetos convexos, particularmente como uma extensão do algoritmo de Gilbert-Johnson-Keerthi (GJK), proposto por \citeonline{gilbert1988fast}. O GJK foi originalmente desenvolvido para computar a distância mínima entre dois conjuntos convexos no espaço tridimensional, sendo posteriormente adaptado para testes de interseção. No entanto, sua formulação não fornece diretamente informações necessárias para a resposta à colisão, como a profundidade de penetração ou a direção de separação.

O desenvolvimento do EPA é geralmente atribuído a \citeonline{vandenbergen1999gjk}, cujos trabalhos estabeleceram bases importantes para a manipulação geométrica do espaço de configuração (CSO). Diferentemente do GJK, que opera sobre um simplex iterativamente ajustado em direção à origem, o EPA parte de um simplex que já contém a origem e o expande progressivamente até aproximar a fronteira do CSO. Apesar de amplamente utilizado, o EPA não foi introduzido por meio de uma única publicação seminal claramente definida, tendo emergido gradualmente a partir de implementações práticas e relatórios técnicos.

Enquanto o BGJK resolve o problema de detecção de colisão ao verificar se a origem pertence ao CSO, o Algoritmo de Expansão de Politopos (EPA, do inglês \textit{Expanding Polytope Algorithm}) resolve o problema dual de encontrar a menor translação necessária para separar os objetos, equivalente ao vetor de menor norma que leva o CSO a não conter mais a origem. Para isso o BGJK pode ser modificado para retornar o simplex final $Q$ em caso de colisão. Dessa forma, é possível usar este simplex para calcular a profundidade e a normal da colisão necessárias para a resposta física, utiliza-se o EPA.

O Algoritmo~\ref{alg:epa} expande o simplex final do BGJK iterativamente até encontrar a fronteira do CSO mais próxima da origem. A distância $\delta$ entre o ponto mais próximo com a origem define a profundidade de penetração. Além disso, o vetor normal $\hat n$ para o ponto mais próximo define a direção de separação. A solução ingênua é usar a normal da face mais próxima da origem, porém um simplex não precisa conter nenhuma das faces do polígono original, o que pode levar à estimativa incorreta da normal de separação.
\begin{algorithm}[htb]
	\caption{EPA}
	\KwIn{Simplex}
	\KwOut{$\hat{n}$, $\delta$}
