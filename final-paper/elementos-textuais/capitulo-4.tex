...
O algoritmo de Expansão de Politopos (EPA, do inglês \textit{Expanding Polytope Algorithm}) surge no contexto da evolução dos métodos de detecção de colisão entre objetos convexos, particularmente como uma extensão do algoritmo de \citeonline{gjk}. O GJK foi originalmente desenvolvido para computar a distância mínima entre dois conjuntos convexos no espaço tridimensional, sendo posteriormente adaptado para testes de interseção. No entanto, sua formulação não fornece diretamente informações necessárias para a resposta à colisão, como a profundidade de penetração ou a direção de separação.
