...
\begin{figure}[htb]
	...
	\caption{Em (A) os objetos delimitadores não se intersectam, em (B) eles se intersectam porém os objetos não se intersectam, em (C) eles se intersectam e os objetos se intersectam. Fonte: Autor.}
	\label{fig:fecho-convexo-interseccao}
\end{figure}

A principal motivação para o uso de objetos delimitadores é que formas mais simples (como caixas ou esferas) permitem testes de sobreposição muito mais baratos do que a geometria original que envolvem. Além disso, se as formas delimitadoras se intersectam, não necessariamente os objetos se intersectam; entretanto, se elas não se intersectam, necessariamente os objetos não se intersectam, como ilustrado na Figura~\ref{fig:fecho-convexo-interseccao}. Esse fato pode ser usado para acelerar os testes de colisão, como veremos posteriormente. 
...
$$
Testes de colisão entre esferas são rápidos, porém inadequados para objetos de proporções irregulares. Elipsoides oferecem melhor ajuste, mas aumentam o custo de teste. Por isso, essas formas são frequentemente utilizadas em fases preliminares da detecção, ou como nós intermediários em hierarquias de volumes delimitadores (BVH, do inglês \textit{Bounding Volume Hierarchy}).

\subsection*{Caixa Delimitadora Alinhada aos Eixos Coordenados}
...
Existem vários algoritmos, entre os quais o \textit{Quickhull}, que é um algoritmo bastante conhecido para o cálculo do fecho convexo de um conjunto finito de pontos em qualquer dimensão, adotando uma estratégia de divisão e conquista semelhante ao \textit{quicksort} \cite{barber1996quickhull}.
...
\end{algorithm}
Apresenta complexidade média $O(n \log n)$ em 2D, assumindo uma distribuição uniforme dos pontos, podendo chegar a $O(n^2)$ em casos degenerados. Em 3D, adapta-se a construções poliedrais mais complexas, mantendo o mesmo princípio recursivo.

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{quickhull_steps.jpg}
    \caption{Etapas do quickhull. Fonte: Adaptado de~\cite{clustering_explanation2024}.}
...
$$
onde $C$ representa o tamanho da célula. Como neste trabalho um objeto é representado por um conjunto de pontos, não temos acesso direto ao seu centro. Portanto, são calculadas as coordenadas de todas as células interceptadas por sua AABB, garantindo a propriedade conservadora da filtragem.

As coordenadas são linearizadas em uma chave única da célula (\textit{cell key}) através de um mapeamento linear simples:
$$Key_{cell} = i + j \times ncol$$
onde $ncol$ é o número dec colunas da grade. Os pares $(Key_{cell}, Index_{obj})$ são salvos num array, onde $Index_{obj}$ representa o índice do objeto no array original ou referência ao objeto na memória.
...
Após a ordenação, os objetos que compartilham a mesma célula aparecem em posições contíguas no array, o quê favorece a localidade espacial e temporal, reduzindo \textit{cache misses} e \textit{page faults}. A partir dessa organização, é possível gerar os pares candidatos à colisão percorrendo o array ordenado e agrupando os objetos por célula. Para evitar testes redundantes, é necessário eliminar dois tipos de duplicatas: (1) se dois objetos A e B caem na mesma célula, é preciso testar apenas o par (A, B) e não repetir para o par (B, A); de forma análoga, (2) se um objeto é grande o suficiente para interceptar múltiplas células, então o par de objetos pode ser gerado mais de uma vez em células distintas, devendo ser testado apenas uma única vez. Essa etapa é essencial para preservar os ganhos de desempenho proporcionados pela filtragem espacial.
...
Em ambientes dinâmicos, nos quais objetos se movem continuamente e podem ser criados ou removidos, a manutenção incremental da estrutura espacial torna-se um aspecto crítico. Embora seja possível atualizar apenas os elementos afetados por cada objeto em movimento, essa abordagem exige controle adicional e estruturas auxiliares. Neste trabalho, opta-se deliberadamente por uma estratégia mais simples: a estrutura é reutilizada a cada passo de tempo a partir de um array linear, cujo contador de elementos é zerado e preenchido novamente com base na ordenação das chaves de célula. Essa decisão representa um trade-off arquitetural, privilegiando simplicidade e previsibilidade de execução em detrimento de possíveis otimizações incrementais.
...
Ao término da filtragem, obtém-se um conjunto $P$ de pares candidatos a colisão, com redução significativa do custo computacional em relação ao método quadrático. Ainda assim, no pior caso, quando muitos objetos se concentram em poucas células, a complexidade pode degradar novamente para $O(n^2)$, evidenciando o caráter heurístico dessa etapa.

A etapa de refinamento realiza testes geométricos precisos nos pares de candidatos em $P$. Essa fase é chamada de Narrow Phase. Utiliza-se algoritmos mais sofisticados para determinar se os objetos estão de fato colidindo, como SAT, GJK e EPA, vistos anteriormente. %Essa fase nem sempre é obrigatória pois, sua aplicação pode se tratar de objetos retangulares que caibam justamente nas dimensões de sua caixa delimitadora.

\section{Como determinar o intervalo de integração da física}

Em simulações físicas, é necessário definir um passo de tempo, usualmente denotado por $dt$ (\textit{Delta Time}), que determina o intervalo entre duas integrações consecutivas do sistema. A escolha desse parâmetro é crítica, pois influencia diretamente tanto a precisão quanto o custo computacional da simulação.

De modo geral, quanto menor o valor de $dt$, mais suave e mais fiel à realidade tende a ser a simulação. Isso ocorre porque o sistema é amostrado com maior frequência, reduzindo erros numéricos e melhor capturando eventos importantes, como colisões. Por outro lado, valores muito pequenos de $dt$ aumentam significativamente o custo computacional, uma vez que, a cada passo, é necessário executar rotinas potencialmente caras, como detecção e resposta a colisões, cujo custo cresce com o número de objetos na cena.

Em cenários com restrições de desempenho, por exemplo, aplicações em tempo real ou com grande quantidade de objetos, pode não ser viável utilizar um $dt$ muito pequeno. Nesses casos, opta-se por um $dt$ maior, reduzindo o número de iterações da simulação por unidade de tempo. Entretanto, essa escolha introduz erros mais significativos. Um problema clássico associado a valores elevados de $dt$ é o fenômeno conhecido como \textit{tunneling}, no qual um objeto pode atravessar outro sem que a colisão seja detectada, pois o instante de contato não é amostrado durante a integração.

Além disso, é importante distinguir entre o passo de tempo da simulação física e o passo de tempo da renderização. Em muitos sistemas, esses dois processos operam com frequências diferentes: a simulação física pode utilizar um $dt$ fixo e relativamente pequeno para garantir estabilidade e reprodutibilidade, enquanto a renderização ocorre com base na taxa de atualização do sistema (frames por segundo), que pode variar dinamicamente. Nesses casos, técnicas de interpolação são frequentemente empregadas para garantir uma visualização suave, mesmo quando a simulação física é executada em uma frequência diferente da renderização.

No contexto da integração numérica, existem dois esquemas principais. O primeiro é a integração com passo fixo, na qual o valor de $dt$ permanece constante ao longo da simulação. Essa abordagem é amplamente utilizada por sua previsibilidade e reprodutibilidade, sendo especialmente importante em aplicações determinísticas, como simulações físicas replicáveis. O segundo esquema é a integração com passo variável, em que o valor de $dt$ é ajustado dinamicamente com base no tempo decorrido entre quadros. Embora essa abordagem seja comum em aplicações interativas, ela pode introduzir inconsistências na simulação, uma vez que diferentes execuções, ou diferentes máquinas, podem produzir resultados distintos devido à variação no número de passos de integração.

Em síntese, a escolha de $dt$ representa um compromisso entre precisão e desempenho. Idealmente, deseja-se utilizar o menor valor possível para garantir estabilidade e exatidão, mas restrições computacionais frequentemente impõem a necessidade de valores maiores, exigindo cuidados adicionais para mitigar erros numéricos e artefatos na simulação.
...
Uma abordagem amplamente adotada consiste na separação conceitual entre as rotinas de renderização e simulação física. Enquanto a renderização é executada na thread principal, geralmente sincronizada com o ciclo de atualização visual por meio de mecanismos como \textit{requestAnimationFrame}
, a simulação física pode ser executada em uma thread separada, utilizando \textit{Web Workers}. Essa separação reduz a probabilidade de bloqueios na interface e permite que o cálculo físico seja realizado de forma assíncrona em relação ao processo de desenho.
...
\begin{figure}[htb]
	...
	\caption{À esquerda esquema tradicional single-thread. À direita esquema com simulação rodando numa thread dedicada a passo fixo. Fonte: Autor.}
...
Por essas razões, muitas engines físicas modernas optam por paralelizar apenas etapas bem definidas e fracamente acopladas da simulação, como a detecção de colisão na fase da filtragem, a resolução de colisões na fase de refinamento e a resposta a colisão em um único thread lógico.
...
Com base nas considerações discutidas, uma arquitetura híbrida mostra-se mais adequada para simulações físicas simplificadas na Web. É importante frisar que, na implementação desenvolvida para este trabalho foi feito apenas a separação da renderização e simulação física em threads distintas, usando \textit{WebWorkers}, sem o uso de WebAssembly ou WebGPU.
% Nessa arquitetura, a renderização é realizada na \textit{main thread}, a simulação física principal é executada em um worker dedicado utilizando WebAssembly, e o paralelismo de dados é explorado por meio de SIMD. Opcionalmente, etapas específicas e fracamente acopladas, como a fase ampla da detecção de colisões, podem ser delegadas à GPU via WebGPU.
